...
def test_witness_openai_responses_non_streaming_shape() -> None:
    proxy = _proxy()
    content = {
        "id": "resp_1",
        "object": "response",
        "output_text": "Paris is the capital of Germany.",
        "output": [
            {
                "type": "message",
                "role": "assistant",
                "content": [{"type": "output_text", "text": "Paris is the capital of Germany."}],
            }
        ],
    }
    ...
    assert headers["X-Entroly-Witness"] == "flagged"
    assert updated["output_text"] != "Paris is the capital of Germany."
    assert updated["output"][0]["content"][0]["text"] == updated["output_text"]
...
def test_witness_openai_responses_streaming_shape_is_parseable() -> None:
    proxy = _proxy()
